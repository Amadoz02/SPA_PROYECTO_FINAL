...
import { get, del, patch, put } from '../../../utils/manejo_api_optimizado.js';
...
export default async function carritoController() {
  ...
  async function updateCartItemQuantity(detalleId, change) {
    try {
      ...
      let paravalidar = await get(`tallas-productos/${currentItem.id_talla_producto}`) // Ajusta según tu estructura de datos
      console.log(paravalidar);
      
      if (newQuantity > paravalidar.stock) {
