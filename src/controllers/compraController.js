...
export default function comprasController(id_usuario = idUser, ids = null) {
  // Elementos del DOM
...
async function cargarCompras() {
  ...
  try {
    if (Array.isArray(ids) && ids.length > 0) {
      for (const id of ids) {
        const res = await fetch(`${base}/ventas/usuario/${id}`);
        const json = await res.json();
        const comprasUsuario = Array.isArray(json)
          ? json.map(c => ({ ...c, id_usuario: id }))
          : [];
        compras.push(...comprasUsuario);
      }

    } else {
      const res = await fetch(`${base}/ventas/usuario/${id_usuario}`);
  ...
  async function verDetalles(id) {
    ...
    toggleLoading(true);
    try {
      const res = await fetch(`${base}/ventas/${id}`);
      if (!res.ok) throw new Error('Error al cargar detalles');
      const json = await res.json();
  
      
      const compra = Array.isArray(json) ? json[0] : json;
  ...
  function mostrarModal(compra) {
    const { usuario, metodoPago, direccion } = compra || {};
              ...
              return `
                <div class="producto-item">
                  ...
                  <div class="producto-info">
                    <h5>${nombre}</h5>
                    <p>Cantidad: ${cant}</p>
                    <p>Talla: <strong>${p.talla}</strong></p>
    ...
    $modalDetails.innerHTML = `
      <div class="detalle-compra">
        ...
        <div class="detalle-info">
          <div class="info-item">
            <strong>Estado:</strong> <span>${compra?.estado ?? '—'}</span>
          </div>
          <div class="info-item">
        ...
        <div class="detalle-direccion">
          <h4>Dirección de envío</h4>
            <p>
            ${escapeHTML(direccion?.direccion || '')}, ${escapeHTML(direccion?.ciudad || '')}, ${escapeHTML(direccion?.departamento || '')}
            </p>
        </div>
